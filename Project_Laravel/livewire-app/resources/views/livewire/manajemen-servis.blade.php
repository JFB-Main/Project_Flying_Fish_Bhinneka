            ...
            <div class="flex flex-col gap-3 shadow-lg">
                <div class="flex flex-row px-10 justify-between">
                    ...
                    <div class="flex flex-row self-end w-fit gap-5">
                        ...
                        <a href="{{ route('add-servis-dps') }}" class="bg-[#0f387a] border border-[#132c53] max-w-fit max-h-fit text-white font-medium p-2 pl-4 pr-4 rounded-lg hover:bg-[#0b4095] cursor-pointer">
                            + Tambah
                        </a>
                    </div>
                </div>
                <div class="flex flex-row shadow-md px-10 pb-5 gap-10">
                    <div class="flex flex-col">
                        <label for="selS" class="text-gray-600">
                            Status
                        </label>
                        <select wire:model.live.debounce.50ms="inputStatus" name="selS" id="ddl" class="w-full min-xl:min-w-[200px] bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5">
                    ...
                    <tbody>
                        @forelse($serviceLogData as $sl)
                            <tr class="border-t hover:bg-blue-50 max-xl:[&>*]:text-[12px] max-lg:[&>*]:text-[10px]">
                                ...
                                @if ($sl->id_teknisi == session(key:'user_id') )
                                    <td class="min-w-40 p-2 pr-25">
                                        <div class="flex flex-col gap-3">
                                            @if ($sl->status != 1 && $sl->status != 4)
                                                <button x-data
                                                        x-on:click="if (confirm('Apakah anda ingin PENDING status tiket ini?')) { $wire.pendingTiketdps({{ $sl->id }}); }"
                                                        type="button" 
                                                        class="text-white min-w-[80px] w-fit px-2 py-1 rounded-lg font-bold bg-yellow-400/90 hover:opacity-60 cursor-pointer">
                                                    @if ($sl->status == 3)
                                                        Continue
                                                    @else
                                                        Pending
                                            ...
                                            @else
                                                <button x-data
                                                        type="button" 
                                                        disabled
                                                        class="text-gray-400 min-w-[80px] w-fit px-2 py-1 rounded-lg border border-yellow-400 cursor-not-allowed">
                                                    Pending
                                                </button>
                                            @endif
                                            @if ($sl->status < 3)
                                                <button x-data
                                                        x-on:click="if (confirm('Apakah anda ingin UPDATE status tiket ini?')) { $wire.updateTiketdps({{ $sl->id }}); }"
                                                        type="button" 
                                                        class="text-white min-w-[80px] w-fit px-2 py-1 rounded-lg font-bold bg-green-500/80 hover:opacity-60 cursor-pointer">
                                                    Update
                                                </button>
                                            @else
                                                <button x-data
                                                        type="button" 
                                                        disabled
                                                        class="text-gray-400 min-w-[80px] w-fit px-2 py-1 rounded-lg border border-green-500 cursor-not-allowed">
                                                    Update
                                                </button>
                                            @endif
                                        </div>
                                ...
                                @else
                                    <td class="min-w-40 p-2 pr-25">
                                        <i class="text-gray-400">Unavailable</i>
                                    </td>
                                @endif
