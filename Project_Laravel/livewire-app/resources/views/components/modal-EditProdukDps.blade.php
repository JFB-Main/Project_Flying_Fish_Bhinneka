...
<div
    id="pop-up"
    x-data = "{ show : false, name : '{{$name}}', modalData: { as_id: null, produk: null }}"
    x-show = "show"
    ...
    <div class="bg-white rounded-b rounded-r rounded-l m-auto fixed inset-0 max-w-xl mb-auto mt-auto h-5/6 max-h-fit shadow-xs shadow-yellow-500 border-t-4 border-[#F8971A]" >
        ...
        <form x-data @submit.prevent="$wire.call('editProduk', modalData.as_id)" class="flex flex-col items-center pt-5 pb-5">
            <div class="flex flex-col gap-2 items-center w-full">
                <div>
                    <h1 class="text-3xl text-cyan-600 font-bold self-center">
                        Edit Product
                    </h1>
                    <p x-text="modalData.product_name ? 'Produk: ' + modalData.product_name + '' : 'Produk tidak ada'" class="text-sm text-gray-600 font-light"></p>
                </div>
                ...
                <div class="flex flex-col gap-10 ">
                    ...
                    <div class="flex flex-row">
                        <label for="editSerialNumber" class="">
                            Serial Number<span class="text-red-500">*</span>
                        </label>
                        <div class="flex flex-col w-full">
                            <input wire:model="editSerialNumber" type="text" name="editSerialNumber" id="editSerialNumber" class="bg-gray-50 border text-gray-900 text-sm rounded-2xl focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('editSerialNumber') border-red-500 @enderror">
                            @error('editSerialNumber')
                                <span class="text-red-500 text-sm">{{ $message }}</span>
                            @enderror
                        </div>
                    </div>
